...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `org.wpilib.aos:aos-sdk` | `headers`                                                                                                                                  | Every header the public AOS libraries need, from AOS, flatbuffers, abseil and tl::expected, including generated ones. Also `cmake/` and `share/aos/`, which are the same on every platform. |
...
single static archive cannot express. Their translation units contribute
nothing but static initializers, so an ordinary static link discards them --
taking `--v` and `--vmodule` with them, since `absl/log:flags` is one of them
and `//aos:init` already depends on it. `AosConfig.cmake` links these whole so a
...
carries the two things that must agree:
