...
## Compile and run the code
  ...
  ```
  bazel build -c opt //ping_pong:pong //
ping_pong:ping //aos:aos_dump
  ```
  ...
  2. In 2 separate windows, run the ping and pong commands using the `pingpong_config.json` config file:
    ...
    2. Run from pre-compiled build (useful for deploying code).
      1. `bazel-bin/aos/events/ping --config bazel-bin/ping_pong/pingpong_config.json`
      2. `bazel-bin/aos/events/pong --config bazel-bin/ping_pong/pingpong_config.json`

  3. In a third window, explore the message stream using `aos_dump`.  Some things you can do:
    1. List the channels:
       `bazel-bin/aos/aos_dump --config bazel-bin/ping_pong/pingpong_config.json`
    2. Listen to a specific topic on a channel-- copy one of the channels listed in the first step and put it at the end of the aos_dump command (e.g., "/test aos.examples.Ping")
       `bazel-bin/aos/aos_dump --config bazel-bin/ping_pong/pingpong_config.json /test aos.examples.Ping`
    3. Listen to multiple topics on a channel (e.g., all the topics published on "/test")
       `bazel-bin/aos/aos_dump --config bazel-bin/ping_pong/pingpong_config.json /test`
...
```
alias aos_dump='bazel-bin/aos/aos_dump --config bazel-bin/ping_pong/pingpong_config.json'
```
...
In addition to running ping and pong, this is a good example to explore event logging.
  ...
  ```
  bazel-bin/aos/events/logging/logger_main --config bazel-bin/ping_pong/pingpong_config.json --logging_folder /tmp/log_folder/
  ```
